...
export const modifierTypes = {
  ...
  VOUCHER_GOLDEN: () => new AddVoucherModifierType(VoucherType.GOLDEN, 1), //바우처 골드 보상 추가
...
const modifierPool: ModifierPool = {
  ...
  [ModifierTier.ULTRA]: [
    ...
    new WeightedModifierType(modifierTypes.VOUCHER_PREMIUM, (party: Pokemon[], rerollCount: integer) => !party[0].scene.gameMode.isDaily ? Math.max(3 - rerollCount, 0) : 0, 3), //바우처 프리미엄과 골드 추가
  ...
  [ModifierTier.MASTER]: [
    ...
    new WeightedModifierType(modifierTypes.VOUCHER_GOLDEN, (party: Pokemon[], rerollCount: integer) => !party[0].scene.gameMode.isDaily && !party[0].scene.gameMode.isEndless && !party[0].scene.gameMode.isSplicedOnly ? Math.max(6 - rerollCount * 2, 0) : 0, 6),
...
function getNewModifierTypeOption(party: Pokemon[], poolType: ModifierPoolType, tier?: ModifierTier, upgradeCount?: integer, retryCount: integer = 0): ModifierTypeOption {
  ...
  if (tier === undefined) {//모디파이어 티어 정하는 부분
